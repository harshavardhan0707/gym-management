...
const Members = () => {
  ...
  const [selectedMember, setSelectedMember] = useState(null); // For edit
  ...
  const fetchMembers = async (pageNum = 1) => {
    ...
    setError(null);
    try {
      const res = await api.get(`/users?page=${pageNum}&limit=${PAGE_SIZE}`);
      setMembers(res.data.users || []);
      setTotal(res.data.pagination?.total || 0);
    } catch (err) {
      setError(
        err.response?.data?.error || "Failed to fetch members. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleAddMemberSubmit = async (data) => {
    ...
    try {
      ...
      setModalError(
        err.response?.data?.error || "Failed to add member. Please try again."
      );
    } finally {
  ...
  const handleEditMemberSubmit = async (data) => {
    ...
    setModalError("");
    try {
      await api.put(`/users/${selectedMember.rollNumber}`, data);
      toast.success("Member updated successfully!");
      handleCloseModal();
      fetchMembers(page);
    } catch (err) {
      setModalError(
        err.response?.data?.error || "Failed to update member. Please try again."
      );
    } finally {
      setModalLoading(false);
    }
  ...
  const handleDeleteMember = async (member) => {
    ...
    try {
      await api.delete(`/users/${member.rollNumber}`);
      toast.success("Member deleted successfully!");
      fetchMembers(page);
    } catch (err) {
      toast.error(
        err.response?.data?.error || "Failed to delete member. Please try again."
      );
    }
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <h2 className="text-4xl font-extrabold text-blue-700 drop-shadow">Members</h2>
        <button
        ...
        <div className="overflow-x-auto rounded-2xl shadow-2xl bg-white border border-blue-100">
          <table className="min-w-full divide-y divide-blue-100">
            <thead className="bg-blue-50">
              <tr>
                <th className="py-4 px-6 font-bold text-blue-700 text-left text-lg">Name</th>
                <th className="py-4 px-6 font-bold text-blue-700 text-left text-lg">Email</th>
                <th className="py-4 px-6 font-bold text-blue-700 text-left text-lg">Phone</th>
                <th className="py-4 px-6 font-bold text-blue-700 text-left text-lg">Roll Number</th>
            ...
            <tbody className="divide-y divide-blue-50">
              {members.length === 0 ? (
                ...
                members.map((member, idx) => (
                  ...
                  >
                    <td className="py-3 px-6 whitespace-nowrap font-medium text-gray-900 text-base">{member.name}</td>
                    <td className="py-3 px-6 whitespace-nowrap text-gray-700 text-base">{member.email}</td>
                    <td className="py-3 px-6 whitespace-nowrap text-gray-700 text-base">{member.phone}</td>
                    <td className="py-3 px-6 whitespace-nowrap text-gray-700 text-base">{member.rollNumber}</td>
